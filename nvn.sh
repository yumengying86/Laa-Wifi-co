...
int=4
# mkdir 10m_100s_nvn_coexist_wifipacketsocket
while(( $int<=4 ))
do
    core_name=`expr $int \- 1`
    taskset -c $core_name nohup ./waf --run "scratch/laa --numA=$int --numB=$int --d2=10 --lbtTxop=5 --duration=100" 2>&1  > ./10m_100s_nvn_coexist_wifipacketsocket/${int}v${int} &
    sleep 1
    let "int++"
done

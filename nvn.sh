...
do
    ...
    taskset -c $core_name nohup ./waf --run "scratch/laa --cellConfigA=Wifi --numA=$int --numB=$int --d2=10 --lbtTxop=5 --phyLogName=_phy_log_${int}v${int} --udpRate=750000000bps --duration=120 --logPhyArrivals=true" 2>&1  > ./10m_120s_nvn_pure_wifi/${int}v${int} &
    sleep 1
    let "int++"
done

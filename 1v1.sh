...
int=1
mkdir 10m_120s_1v1_lbttxop
while(( $int<=8 ))
do
    core_name=`expr $int \+ 2`
    taskset -c $core_name nohup ./waf --run "scratch/laa --numA=1 --numB=1 --d2=10 --lbtTxop=$int --phyLogName=_phy_log_1v1_lbttxop_$int --udpRate=750000000bps --duration=120 --logPhyArrivals=true" 2>&1  > ./10m_120s_1v1_lbttxop/lbttxop$int &
    sleep 1
    let "int++"
done

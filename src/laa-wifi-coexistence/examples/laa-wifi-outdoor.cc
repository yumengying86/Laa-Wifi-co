...
NS_LOG_COMPONENT_DEFINE ("LaaWifiOutdoor");
...
main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::RadioEnvironmentMapHelper::OutputFile", StringValue ("laa-wifi-outdoor.rem"));
 ...
 GlobalValue::GetValueByName ("asciiEnabled", booleanValue);
  ...
  if (generateRem)
    {
      ...
      std::string topologyFileName = remDir + "/laa-wifi-outdoor-topology.gnuplot";
  ...
  ConfigureAndRunScenario (cellConfigA, cellConfigB, bsNodesA, bsNodesB, ueNodesA, ueNodesB, phyParams, durationTime, transport, "ns3::ItuUmiPropagationLossModel", disableApps, lteDutyCycle, generateRem, outputDir + "/laa_wifi_outdoor_" + simTag, simulationParams.str ());

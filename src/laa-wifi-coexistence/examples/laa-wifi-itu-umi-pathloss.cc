...
int main (int argc, char** argv)
{
  ...
  outFile.open ("laa-wifi-itu-umi-pathloss.gnuplot", std::ofstream::out | std::ofstream::trunc);

  GnuplotCollection gcRxPowerPdf ("laa-wifi-itu-umi-rx-power.pdf");
  gcRxPowerPdf.AddPlot (rxPowerPlot);
  gcRxPowerPdf.GenerateOutput (outFile);

  GnuplotCollection gcRxPowerPng ("laa-wifi-itu-umi-rx-power.png");
  gcRxPowerPng.AddPlot (rxPowerPlot);
  gcRxPowerPng.GenerateOutput (outFile);

  GnuplotCollection gcPLosPdf ("laa-wifi-itu-umi-plos.pdf");
  gcPLosPdf.AddPlot (pLosPlot);
  gcPLosPdf.GenerateOutput (outFile);

  GnuplotCollection gcPLosPng ("laa-wifi-itu-umi-plos.png");
